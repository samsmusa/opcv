...
def get_sheet_upload_file_path(instance, result=False):
    exam_id = instance.exam_id
    section_id = instance.section_id
    subject_id = instance.subject_id
    _filename = f'/{settings.MEDIA_ROOT}/{exam_id}/{section_id}/{subject_id}/{"outputs" if result else "inputs"}/'

    return os.path.join(settings.BASE_DIR, _filename)
...
class OmrScan(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        file = obj.file.open()
        file_input_path = get_sheet_upload_file_path(obj)
        file_out_path = get_sheet_upload_file_path(obj, result=True)
        with zipfile.ZipFile(file, 'r') as zip_ref:
            zip_ref.extractall(file_input_path)
        data = entry_point_for_args(
            {'input_paths': [file_input_path],
        ...
        return Response({"path": file_input_path})
